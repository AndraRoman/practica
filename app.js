...
function xzero($http, game, userName) {
    ...
    vm.status = function () {
        $http({
            ...
            vm.players = response.data.players;//actualizam lista, mutarile etc sau cine a solicitat
            response.data.games.forEach(function(item){
                if(item.stare==='init')
                    if(confirm(game.player1+'vrea sa joace c u tine. ok?')) vm.games.push(game.create(item));
